	...
	for d in range(0, len(Docs)):
		...
		for k in xrange(0,topics):
			...
			for w in Docs[d]:
				if w not in wordMap:
					continue
				val += Pwz[k][wordMap[w]] * Docs[d].count(w)*1.0/len(Docs[d])
			temp.append(val)
		Pdz.append(temp)
	return Pdz
...
def NMI(clusters, labels, n):
	...
	for i in range(0, len(clusters)):
		H_pred += (len(clusters[i])*1.0/n) *math.log(len(clusters[i])*1.0/n)
	for j in range(0, len(labels)):
		H_labels += (len(labels[j])*1.0/n) *math.log(len(labels[j])*1.0/n)
	Inf = 0.0
	for i in range(0, len(clusters)):
		for j in range(0, len(labels)):
			intersection = len(set(clusters[i]) & set(labels[j]))
			if intersection != 0:
				Inf += intersection*math.log(1.0*len(clusters[i]) * len(labels[j])/(n*intersection))/n
